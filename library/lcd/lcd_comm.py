...
class LcdComm(ABC):
    ...
    def openSerial(self):
        ...
        # print(f"Initial port status:")
        # initial_status = check_serial_status(self.lcd_serial)
    ...
    def serial_write(self, data: bytes):
        packet_size = 64  # USB packet size

        # Send all odd-indexed packets first
        for i in range(0, len(data), packet_size * 2):
            chunk = data[i : i + packet_size]
            self.lcd_serial.write(chunk)
            self.lcd_serial.flush()
            time.sleep(0.0001)

        # Very short pause to avoid frame reset
        # time.sleep(0.0002)  # Try much shorter pause

        # Then send all even-indexed packets
        for i in range(packet_size, len(data), packet_size * 2):
            chunk = data[i : i + packet_size]
            self.lcd_serial.write(chunk)
            self.lcd_serial.flush()
            time.sleep(0.0001)
